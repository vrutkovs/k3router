...
mkdir -p workdir
rm -f workdir/k3router.iso || true
...
docker run --interactive --pull=always --rm -v .:/data -w /data --security-opt label=disable quay.io/coreos/butane:release --pretty --strict -d /data -o /data/workdir/config.ign butane.yaml
docker run --interactive --privileged --rm -v ./workdir:/data -w /data --entrypoint=bash quay.io/coreos/coreos-installer:release -c "coreos-installer iso customize \
    ...
    -o /data/k3router.iso /data/fedora-coreos-*-live.x86_64.iso"
echo "Wrote $(pwd)/k3router.iso"
